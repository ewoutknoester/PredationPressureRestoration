...
# Load raw data
db.0raw <- read_excel("Raw data/Fish surveys_DATABASE_04_Selection.xlsx", sheet = "SPECIES")
...
db.2Fac.sur  <- db.2Fac %>%
        ...
        dplyr::summarise(Biomass.sum.kgha = sum(Biomass))

# Merge to get surveys where biomass = 0 back
db.2Fac.sur <- merge(db.2surveys, db.2Fac.sur, by = 'Survey', all.x = TRUE)
db.2Fac.sur[is.na(db.2Fac.sur)] <- 0
...
# Merge to get surveys where biomass = 0 back
db.2Obl.sur <- merge(db.2surveys, db.2Obl.sur, by = 'Survey', all.x = TRUE)
db.2Obl.sur[is.na(db.2Obl.sur)] <- 0
